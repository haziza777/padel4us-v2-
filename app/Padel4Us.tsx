...
type Profile = {
  id: string; name: string; email?: string; level: string; area: string;
  hand: string; side: string; style?: string; bio?: string; avatar_url?: string;
  confirmations?: number; badges?: string[]; video_count?: number;
};

type Game = {
  id: string; host_id: string; host_name?: string; venue: string;
  game_date: string; game_time: string; spots: number; level: string;
  urgent: boolean; joined?: string[]; player_ids?: string[];
};

type Video = {
  id: string; user_id: string; player_name?: string; video_url: string;
  description: string; privacy: string; likes_count: number;
...
type Group = {
  ...
  members: string[]; video_count: number;
};
...
const btn1: React.CSSProperties = {
  ...
  cursor:"pointer",boxShadow:"0 4px 15px rgba(0,180,216,.3)",
...
export default function Padel4Us() {
  ...
  const loadData = useCallback(async () => {
    ...
    if (vv) setVideos(vv.map((v:Video)=>{const p=pp.find((p:Profile)=>p.id===v.user_id);return{...v,player_name:p?.name||"?"}}));
    ...
    if (grp) {
      const eg2 = await Promise.all(grp.map(async (g:Group) => {
        ...
        const mn = mids.map((id:string)=>pp.find((p:Profile)=>p.id===id)?.name||"").filter(Boolean);
  ...
  const doLogout = async () => {
    ...
    setMe(null); setUserId(null); setScreen("auth"); setAuthMode("login");
  ...
  const doConfirmLevel = async (pid: string) => {
  ...
  const doShare = async (v: Video) => {
    ...
    if (navigator.share) {
      try { await navigator.share({title:"padel4us 🎾",text:v.description,url}); } catch{}
    } else {
      try { await navigator.clipboard.writeText(url); alert("הלינק הועתק!"); } catch { alert(url); }
    }
  };
  ...
  const openComments = async (v: Video) => {
    ...
    if (data) {
      setComments(data.map((c:Comment)=>{
        const p = players.find(x=>x.id===c.user_id);
        return {...c, user_name: p?.name||"?"};
      }));
    } else setComments([]);
  };

  const doAddComment = async () => {
    if (!newCmt.trim()||!userId||!mComments) return;
    await supabase.from("comments").insert({video_id:mComments.id,user_id:userId,content:newCmt.trim()});
    setNewCmt("");
  ...
  const fGames = games.filter(g => {
    if (fArea!=="הכל" && !g.venue.includes(fArea)) return false;
    if (fLevel!=="הכל" && g.level!==fLevel && g.level!=="כל הרמות") return false;
    return true;
  });
  ...
  const Chip = ({opts,val,set,c="#00b4d8"}:{opts:string[];val:string;set:(v:string)=>void;c?:string}) => (
  ...
  const Mod = ({close,children}:{close:()=>void;children:React.ReactNode}) => (
    <div onClick={close} style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:200}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#1a2634",borderRadius:"20px 20px 0 0",padding:"24px 20px 40px",width:"100%",maxWidth:500,maxHeight:"85vh",overflowY:"auto"}}>{children}</div>
    </div>
  );
    ...
    if (authMode==="setup") return (
      <div style={{height:"100vh",background:"#0a0e17",overflowY:"auto",padding:"40px 24px",direction:"rtl",fontFamily:"'Heebo',sans-serif"}}>
        ...
        <div style={{textAlign:"center",marginBottom:30}}>
          <div style={{width:90,height:90,borderRadius:"50%",background:"linear-gradient(135deg,#00b4d8,#0077b6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:45,margin:"0 auto 10px",boxShadow:"0 4px 20px rgba(0,180,216,.3)"}}>😎</div>
        </div>
        {[
          {l:"רמת משחק",c:<><Chip opts={["2.5","3.0","3.5","4.0","4.5","5.0"]} val={setupLevel} set={setSetupLevel}/><div style={{color:"rgba(255,255,255,.25)",fontSize:11,marginTop:6}}>חברים יוכלו לאשר</div></>},
          {l:"אזור",c:<Chip opts={S_AREAS} val={setupArea} set={setSetupArea} c="#e9c46a"/>},
          {l:"יד דומיננטית",c:<Chip opts={["ימני","שמאלי"]} val={setupHand} set={setSetupHand} c="#2a9d8f"/>},
          {l:"צד מועדף",c:<Chip opts={["ימין","שמאל","שניהם"]} val={setupSide} set={setSetupSide} c="#e76f51"/>},
          {l:"סגנון",c:<Chip opts={["התקפי","הגנתי","מאוזן"]} val={setupStyle} set={setSetupStyle} c="#a78bfa"/>},
        ].map((x,i)=><div key={i} style={{marginBottom:20}}><div style={{color:"rgba(255,255,255,.6)",fontSize:13,fontWeight:600,marginBottom:8}}>{x.l}</div>{x.c}</div>)}
      ...
      <div style={{flex:1,overflowY:"auto",scrollSnapType:"y mandatory" as const}}>
        {videos.length===0?(
          <div style={{height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",color:"rgba(255,255,255,.3)",padding:40,textAlign:"center"}}>
            ...
            <div style={{fontSize:18,fontWeight:600,color:"rgba(255,255,255,.5)",marginBottom:8}}>עדיין אין סרטונים</div>
            <div style={{fontSize:14,marginBottom:20}}>היה הראשון!</div>
          ...
          <div key={v.id} style={{width:"100%",height:"calc(100dvh - 120px)",scrollSnapAlign:"start" as const,background:"#000",position:"relative",display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
            ...
            <div style={{position:"absolute",left:12,bottom:120,display:"flex",flexDirection:"column",gap:20,alignItems:"center",zIndex:5}}>
              <div style={{textAlign:"center",cursor:"pointer"}} onClick={()=>doLike(v.id)}>
                <div style={{fontSize:26}}>{liked[v.id]?"❤️":"🤍"}</div>
                <div style={{color:"#fff",fontSize:11,marginTop:2}}>{v.likes_count+(liked[v.id]?1:0)}</div>
              </div>
              <div style={{textAlign:"center",cursor:"pointer"}} onClick={()=>openComments(v)}>
                <div style={{fontSize:24}}>💬</div>
                <div style={{color:"#fff",fontSize:11,marginTop:2}}>{v.comments_count}</div>
              </div>
              <div style={{textAlign:"center",cursor:"pointer"}} onClick={()=>doShare(v)}>
  ...
  const Prof = () => {
    const p = selPlayer||me; if(!p) return null;
    const own = !selPlayer;
    const e = players.find(x=>x.id===p.id)||p;
    const pv = videos.filter(v=>v.user_id===e.id);
    return (
      <div style={{height:"100%",overflowY:"auto",background:"#0a0e17"}}>
        <div style={{background:"linear-gradient(135deg,#0f1923,#1a2634)",padding:"30px 20px 24px",textAlign:"center",borderBottom:"1px solid rgba(255,255,255,.06)",position:"relative"}}>
          ...
          {own&&<div onClick={()=>{setEName(p.name);setEArea(p.area);setELevel(p.level);setEHand(p.hand);setESide(p.side);setEStyle(p.style||"");setEBio(p.bio||"");setMEditProf(true);}} style={{position:"absolute",top:16,left:16,cursor:"pointer",color:"rgba(255,255,255,.5)",fontSize:13}}>✏️ ערוך</div>}
          ...
          <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(0,180,216,.15)",border:"1px solid rgba(0,180,216,.3)",borderRadius:20,padding:"4px 14px",marginBottom:8}}>
            <span style={{color:"#00b4d8",fontSize:13,fontWeight:600}}>רמה {e.level}</span>
            <span style={{color:"rgba(255,255,255,.4)",fontSize:12}}>•</span>
            <span style={{color:"rgba(255,255,255,.5)",fontSize:12}}>{e.confirmations||0} אישרו ✓</span>
          </div>
          <div style={{color:"rgba(255,255,255,.5)",fontSize:13,marginTop:4}}>📍 {e.area} • {e.hand} • צד {e.side}</div>
          {e.bio&&<div style={{color:"rgba(255,255,255,.4)",fontSize:12,marginTop:8,maxWidth:280,margin:"8px auto 0"}}>{e.bio}</div>}
          {e.style&&<div style={{display:"inline-block",marginTop:8,background:"rgba(167,139,250,.15)",border:"1px solid rgba(167,139,250,.3)",borderRadius:12,padding:"3px 10px",color:"#a78bfa",fontSize:11}}>סגנון: {e.style}</div>}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",borderBottom:"1px solid rgba(255,255,255,.06)"}}>
          {[{v:e.video_count||0,l:"סרטונים"},{v:e.confirmations||0,l:"אישורים"},{v:e.badges?.length||0,l:"תוויות"}].map((s,i)=>(
            <div key={i} style={{padding:"16px 0",textAlign:"center",borderLeft:i>0?"1px solid rgba(255,255,255,.06)":"none"}}>
              <div style={{color:"#fff",fontSize:20,fontWeight:700}}>{s.v}</div>
              <div style={{color:"rgba(255,255,255,.4)",fontSize:12,marginTop:2}}>{s.l}</div>
            </div>
          ))}
        </div>
        {e.badges&&e.badges.length>0&&<div style={{padding:"16px 20px"}}><div style={{color:"rgba(255,255,255,.5)",fontSize:12,marginBottom:10,fontWeight:600}}>תוויות</div><div style={{display:"flex",flexWrap:"wrap",gap:8}}>{e.badges.map((b,i)=><span key={i} style={{background:"rgba(255,255,255,.06)",border:"1px solid rgba(255,255,255,.1)",borderRadius:20,padding:"6px 14px",color:"#e0e0e0",fontSize:13}}>{b}</span>)}</div></div>}
        <div style={{padding:"16px 20px 20px"}}>
          ...
          {pv.length===0?(
            <div style={{textAlign:"center",padding:40,border:"2px dashed rgba(255,255,255,.1)",borderRadius:16,color:"rgba(255,255,255,.3)"}}>
              ...
              {own&&<button onClick={()=>setMUpload(true)} style={{marginTop:12,padding:"8px 20px",borderRadius:20,background:"rgba(0,180,216,.15)",border:"1px solid rgba(0,180,216,.3)",color:"#00b4d8",fontSize:13,fontWeight:600,cursor:"pointer"}}>העלה סרטון ראשון</button>}
            </div>
          ):(
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:4}}>
              {pv.map((v,i)=>(
                <div key={v.id} style={{aspectRatio:"9/16",borderRadius:8,background:"#000",cursor:"pointer",position:"relative",overflow:"hidden"}} onClick={()=>setMVideo(v)}>
                  ...
                  {own&&<div onClick={ev=>{ev.stopPropagation();doDeleteVideo(v.id,v.video_url);}} style={{position:"absolute",top:4,left:4,width:24,height:24,borderRadius:"50%",background:"rgba(230,57,70,.9)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:11,cursor:"pointer",zIndex:5}}>✕</div>}
                </div>
              ))}
            </div>
          )}
        </div>
        {!own&&<div style={{padding:"0 20px 30px",display:"flex",gap:10}}>
          <button style={{...btn1,flex:1}}>🎾 הזמן למשחק</button>
          <button onClick={()=>doConfirmLevel(e.id)} style={{flex:.6,padding:"14px 0",borderRadius:12,border:"1px solid rgba(0,180,216,.3)",background:"transparent",color:"#00b4d8",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:"'Heebo',sans-serif"}}>✓ אשר רמה</button>
        </div>}
        {own&&<div style={{padding:"0 20px 30px"}}><button onClick={doLogout} style={{width:"100%",padding:"12px 0",borderRadius:12,border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"rgba(255,255,255,.4)",fontSize:13,cursor:"pointer",fontFamily:"'Heebo',sans-serif"}}>התנתק</button></div>}
      </div>
    );
  };
  ...
  const GCard = ({g,u}:{g:Game;u:boolean}) => (
    <div onClick={()=>setMGameDet(g)} style={{background:u?"linear-gradient(135deg,rgba(230,57,70,.1),rgba(230,57,70,.05))":"rgba(255,255,255,.04)",border:`1px solid ${u?"rgba(230,57,70,.25)":"rgba(255,255,255,.08)"}`,borderRadius:14,padding:"14px 16px",marginBottom:8,cursor:"pointer"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div>
          <div style={{color:"#fff",fontSize:15,fontWeight:700}}>{g.venue}</div>
          <div style={{color:"rgba(255,255,255,.5)",fontSize:13,marginTop:4}}>{g.game_date} • {g.game_time} • חסר {g.spots} {g.spots===1?"שחקן":"שחקנים"}</div>
          <div style={{color:"rgba(255,255,255,.4)",fontSize:12,marginTop:4}}>מארגן: {g.host_name} • רמה {g.level}</div>
          {g.joined&&g.joined.length>0&&<div style={{color:"rgba(255,255,255,.3)",fontSize:11,marginTop:4}}>בפנים: {g.joined.join(", ")}</div>}
        </div>
        <button onClick={ev=>{ev.stopPropagation();doJoinGame(g.id);}} style={{padding:"8px 18px",borderRadius:10,border:"none",background:joinedG[g.id]?"rgba(255,255,255,.1)":u?"#e63946":"linear-gradient(135deg,#00b4d8,#0077b6)",color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",flexShrink:0,fontFamily:"'Heebo',sans-serif"}}>{joinedG[g.id]?"✓ הצטרפת":u?"הצטרף!":"הצטרף"}</button>
      </div>
    </div>
  );
  ...
  const Games = () => (
    <div style={{height:"100%",overflowY:"auto",background:"#0a0e17"}}>
      ...
      {urgGames.length>0&&<div style={{padding:"0 20px 12px"}}><div style={{color:"#e63946",fontSize:13,fontWeight:700,marginBottom:10,display:"flex",alignItems:"center",gap:6}}><span style={{animation:"pulse 1.5s infinite"}}>🔴</span> צריך אותך עכשיו!</div>{urgGames.map(g=><GCard key={g.id} g={g} u={true}/>)}</div>}
      <div style={{padding:"0 20px 100px"}}>
        <div style={{color:"rgba(255,255,255,.5)",fontSize:13,fontWeight:700,marginBottom:10}}>כל המשחקים</div>
        {fGames.length===0?<div style={{textAlign:"center",padding:40,color:"rgba(255,255,255,.3)"}}>{games.length===0?"עדיין אין משחקים":"אין משחקים מתאימים"}</div>:fGames.map(g=><GCard key={g.id} g={g} u={false}/>)}
      </div>
  ...
  const Community = () => (
    <div style={{height:"100%",overflowY:"auto",background:"#0a0e17"}}>
      <div style={{padding:20}}>
        ...
        {players.length>0&&<>
          <div style={{color:"rgba(255,255,255,.5)",fontSize:13,fontWeight:600,marginTop:30,marginBottom:12}}>שחקנים באפליקציה</div>
          <div style={{display:"flex",gap:16,overflowX:"auto",paddingBottom:8}}>
            {players.filter(p=>p.id!==userId).slice(0,8).map(p=>(
              <div key={p.id} onClick={()=>{setSelPlayer(p);setTab("profile");}} style={{textAlign:"center",cursor:"pointer",flexShrink:0}}>
                ...
                <div style={{color:"rgba(255,255,255,.5)",fontSize:11,maxWidth:60,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name.split(" ")[0]}</div>
              </div>
            ))}
          </div>
        </>}
  ...
  return (
    <div style={{width:"100%",maxWidth:500,margin:"0 auto",height:"100dvh",background:"#0a0e17",fontFamily:"'Heebo','Segoe UI',sans-serif",direction:"rtl",overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"}}>
      ...
      <div style={{background:"rgba(10,14,23,.97)",backdropFilter:"blur(12px)",borderTop:"1px solid rgba(255,255,255,.08)",display:"flex",justifyContent:"space-around",padding:"8px 0 16px",flexShrink:0,zIndex:10}}>
        {[{id:"home",i:"🏠",l:"בית"},{id:"search",i:"🔍",l:"חיפוש"},{id:"games",i:"🎾",l:"משחקים"},{id:"community",i:"👥",l:"קהילה"},{id:"profile",i:"👤",l:"פרופיל"}].map(t=>(
          <div key={t.id} onClick={()=>{setTab(t.id);if(t.id==="profile")setSelPlayer(null);}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,cursor:"pointer",opacity:tab===t.id?1:.4,transition:"opacity .2s"}}>
            ...
            <span style={{fontSize:10,fontWeight:600,color:tab===t.id?"#00b4d8":"rgba(255,255,255,.5)"}}>{t.l}</span>
          </div>
        ))}
      </div>
      ...
      {/* Comments */}
      {mComments&&<Mod close={()=>setMComments(null)}>
        ...
        <div style={{display:"flex",gap:8}}>
          <input style={{...inp,flex:1}} placeholder="כתוב תגובה..." value={newCmt} onChange={e=>setNewCmt(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")doAddComment();}}/>
          <button onClick={doAddComment} disabled={!newCmt.trim()} style={{padding:"0 20px",borderRadius:12,border:"none",background:newCmt.trim()?"linear-gradient(135deg,#00b4d8,#0077b6)":"rgba(255,255,255,.1)",color:"#fff",fontSize:14,fontWeight:600,cursor:newCmt.trim()?"pointer":"default",fontFamily:"'Heebo',sans-serif",flexShrink:0}}>שלח</button>
        </div>
      </Mod>}

      {/* Upload */}
      {mUpload&&<Mod close={()=>setMUpload(false)}>
        ...
        <div style={{marginBottom:20}}>
          <div style={{color:"rgba(255,255,255,.5)",fontSize:12,marginBottom:8}}>פרטיות</div>
          <div style={{display:"flex",gap:10}}>
            {[{k:"group",l:"🔒 קבוצה"},{k:"public",l:"🌍 ציבורי"}].map(p=><button key={p.k} onClick={()=>setUpPriv(p.k)} style={{flex:1,padding:"10px 0",borderRadius:10,border:"1px solid",borderColor:upPriv===p.k?"#00b4d8":"rgba(255,255,255,.1)",background:upPriv===p.k?"rgba(0,180,216,.12)":"transparent",color:upPriv===p.k?"#00b4d8":"rgba(255,255,255,.4)",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"'Heebo',sans-serif"}}>{p.l}</button>)}
          </div>
        </div>
      ...
      {/* New Game */}
      {mNewGame&&<Mod close={()=>setMNewGame(false)}>
      ...
      {/* Game Details */}
      {mGameDet&&<Mod close={()=>setMGameDet(null)}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
          {[{l:"מארגן",v:mGameDet.host_name||""},{l:"רמה",v:mGameDet.level},{l:"פנויים",v:`${mGameDet.spots}`},{l:"בפנים",v:`${mGameDet.joined?.length||0}`}].map((x,i)=><div key={i} style={{background:"rgba(255,255,255,.04)",borderRadius:10,padding:"10px 14px"}}><div style={{color:"rgba(255,255,255,.4)",fontSize:11}}>{x.l}</div><div style={{color:"#fff",fontSize:14,fontWeight:600,marginTop:2}}>{x.v}</div></div>)}
        </div>
      ...
      {/* Edit Profile */}
      {mEditProf&&<Mod close={()=>setMEditProf(false)}>
